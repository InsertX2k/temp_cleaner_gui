...
class ControllableThread(threading.Thread):
    ...
    # overriding thread's run() method to add exception handling for
    # our custom defined thread termination exceptions
    def run(self):
        """
        override for super class's method run()
        """
        try:
            super().run() # returns nothing so we don't care about its return value
            return
        except error.UserInitiatedThreadStop:
            return
        except error.AutomatedThreadTermination:
            return
        except Exception as __runException:
            tb = __runException.__traceback__
            bcodeFromTb: dis.Bytecode = dis.Bytecode.from_traceback(tb)
            print(f"""[ERROR] {threading.current_thread().name} - {threading.current_thread().ident}: Execution failed!
Error details: {__runException}
                  
Bytecode Instructions:
{bcodeFromTb.dis()}

Detailed Traceback:
{traceback.format_stack(tb.tb_frame)}

This error should be shown to the user via an error window.
""")
            error.ErrorWindow(f"""An Exception has occured during the execution of the thread "{threading.current_thread().name}":
Error details: {__runException}
...
""")
        # we don't need to define exception handler for other exception types
        # since we already want them to be thrown at us.
...
class MainWindowLightMode(CTk):
    def __init__(self):
        ...
        # an event that we will use.
        self.__canDestroySelf: threading.Event = threading.Event()
        self.__canDestroySelf.clear()
        # shutdown poller function
        def __poll_shutdown_evt():
            if self.__canDestroySelf.is_set():
                return __class__.__bases__[0].destroy(self) # super class's destroy method.
            self.after(500, __poll_shutdown_evt) # reschedule the wait.
            return None
...
""")
        ...
        self.sharedObjsCSectionsAccessLocker = threading.Lock()
        with self.sharedObjsCSectionsAccessLocker:
        ...
        def execute_theprogram():
            ...
            **THIS FUNCTION MUST BE RUNNING IN A SEPARATE THREAD, OTHER THAN THE MAIN PROGRAM THREAD**
            ...
            if str(GetConfig['ProgConfig']['AcquireWakeLock']) == '1':
                ...
                self.sharedObjsCSectionsAccessLocker.acquire()
                if self.isWakeLockAcquired:
                    self.sharedObjsCSectionsAccessLocker.release()
        ...
        # let's define the function that will be executed by the thread that does the tasks for safely
        # exiting the program's process.
        def __safely_terminate_program_process_worker() -> None:
            """
            The function that will be executed by the thread that ensures safe termination of this program's
            processes/threads.

            **THIS FUNCTION MUST BE EXECUTED UNDER A DIFFERENT THREAD ASIDE FROM THE MainThread, SINCE IT WILL JOIN THREADS**

            This function will do all of the following (at the very least):
            * Acquire the Lock for shared objects to safely set the control variable for the Sys Usr animation
            * Check to see if the Cleaning Thread is running or not, if it's running, it will attempt to terminate it (Bug fix)
            * Call the overridden method `destroy()` which in turn ensures there aren't any running animations on the animated widgets.
            * Join the Thread spawned by that overridden `destroy()` method, of course after checking whether or not it is running.
            """
            FUNCTION_NAME: str = "__safely_terminate_program_process_worker"
            print(f"[DEBUG] {threading.current_thread().name}:{threading.current_thread().ident}@{FUNCTION_NAME}: execution begin...")
            # First Task - Acquire the Lock that protects shared objects and set the control variable
            # for the Sys Usr Warn Animation to False to instruct that thread to safely terminate.
            print(f"[DEBUG] {threading.current_thread().name}:{threading.current_thread().ident}@{FUNCTION_NAME}: " + 
                  "trying to acquire Shared Objects Critical Sections Access Locker Lock...")
            with self.sharedObjsCSectionsAccessLocker:
                print(f"[DEBUG] {threading.current_thread().name}:{threading.current_thread().ident}@{FUNCTION_NAME}: " + 
                      "successfully acquired the Lock, setting control variable for the Sys Usr Warn Animation to False...")
                self.__runSysUsrWarnAnimation = False
            print(f"[DEBUG] {threading.current_thread().name}:{threading.current_thread().ident}@{FUNCTION_NAME}: " + 
                  "successfully set the control variable for Sys Usr Warn Animation to False, and released the Lock!")
            # why don't we join the sys usr warn animation thread (just in case??)
            print(f"[DEBUG] {threading.current_thread().name}:{threading.current_thread().ident}@{FUNCTION_NAME}: " + 
                  "Joining the Sys Usr Warn Animation Thread (just in case)...")
            # let's check if it's alive first.
            if self.chkIfUsingSysUsrThd.is_alive():
                self.chkIfUsingSysUsrThd.join()
            else: # for printing to the debugging console ONLY.
                print(f"[DEBUG] {threading.current_thread().name}:{threading.current_thread().ident}@{FUNCTION_NAME}: " + 
                      "Sys Usr Warn Animation Thread was not running!")
            # Bonus Task - Why don't we just join the thread used to update the banner with Tmp dirs sizes
            # this way, I won't have to fix this bug in the future.
            print(f"[DEBUG] {threading.current_thread().name}:{threading.current_thread().ident}@{FUNCTION_NAME}: " + 
                  "Joining the Tmp dirs sizes check thread (Just in case)...")
            if self.updBannerTmpFsThd.is_alive():
                self.updBannerTmpFsThd.join()
            else: # for printing to debugging console ONLY.
                print(f"[DEBUG] {threading.current_thread().name}:{threading.current_thread().ident}@{FUNCTION_NAME}: " +
                      "Tmp dirs sizes check thread was not running!")
            # I know you may be wondering at this time, why don't I do the same thing to the thread used to check for updates,
            # and I'm going to tell you, that's not necessary, why?, because that thread can't cause a deadlock because I've 
            # previously set a timeout for it, also it doesn't do any UI updates, so there's nothing to worry about from its side.
            # Additional Bonus Task (includes a bug fix btw) - Check to see if the cleaning thread is running and terminate its
            # activity if it is currently running.
            print(f"[DEBUG] {threading.current_thread().name}:{threading.current_thread().ident}@{FUNCTION_NAME}: " +
                  "Checking if the Cleaning Thread is running or not...")
            while self.execbtn_thread.is_alive():
                print(f"[DEBUG] {threading.current_thread().name}:{threading.current_thread().ident}@{FUNCTION_NAME}: " +
                      "Cleaning thread is running, will send termination signal to it...")
                try: self.execbtn_thread.raiseExc()
                except Exception as __errFailureInStoppingCleaningThd:
                    print(f"[ERROR] {threading.current_thread().name}:{threading.current_thread().ident}@{FUNCTION_NAME}: " +
                          f"Failed to send termination signal to the Cleaning Thread: {__errFailureInStoppingCleaningThd}")
                    break # we will break for now because it could be already terminated (who knows?)
            # if we exit this loop, it means that we have terminated the cleaning thread or not.
            # anyway for now it will be safe to continue...
            # Second Task - Call the overridden self's destroy() method.
            print(f"[DEBUG] {threading.current_thread().name}:{threading.current_thread().ident}@{FUNCTION_NAME}: " + 
                  "Instructing the main UI window to destroy itself via calling self.destroy()...")
            self.destroy() # that's the overridden self.destroy() method.
            # Third Task - Join the thread that spawned by calling that self.destroy() method.
            print(f"[DEBUG] {threading.current_thread().name}:{threading.current_thread().ident}@{FUNCTION_NAME}: " + 
                  "Joining the thread spawned by self.destroy()...")
            # but first we must check whether or not it is running.
            if self.__safeDestroyEvtHandlerThdObj.is_alive(): # returns True if thread is alive
                self.__safeDestroyEvtHandlerThdObj.join() # joining that thread.
            else: # only for printing to debugging console.
                print(f"[DEBUG] {threading.current_thread().name}:{threading.current_thread().ident}@{FUNCTION_NAME}: " + 
                      "The thread spawned by self.destroy() was not probably running.")
            
            # at this point, all tasks have been completed and we do not need to do anything else, therefore its just safe
            # to end the execution of this function and this thread at this point.
            print(f"[DEBUG] {threading.current_thread().name}:{threading.current_thread().ident}@{FUNCTION_NAME}: all tasks finished!" + 
                  ", terminating this thread safely...")
            # this function should ALWAYS return None.
            return None


        def close_main_screen() -> None:
            FUNCTION_NAME: str = "close_main_screen"
            """
            A function that will start the safe main UI window exit thread to do necessary tasks to ensure safe 
            exit for the program's process.

            It is the responsibility of this function to make sure this thread won't be started more than once.
            """
            print(f"[DEBUG] {FUNCTION_NAME}: User has sent the command WM_DESTROY_WINDOW, will run tasks for safe termination of this program's process...")
            # we must check if it's already running or not before attempting to start it.
            __thds_objs: list[threading.Thread] = threading.enumerate()
            for __thd_obj in __thds_objs:
                print(f"[DEBUG] {FUNCTION_NAME}: Thread {str(__thd_obj.name)}")
                if str(__thd_obj.name) == self.safelyTerminateProcessWorkerThdName:
                    print(f"[DEBUG] {FUNCTION_NAME}: Safe Process Termination Worker Thread is already running, will ignore this event...")
                    return None
            # if we exit this loop and the program counter reaches this point, it means the thread is not running.
            # but still, we need to implement exception handling, since the start() method raises a RuntimeError if 
            # the thread was already running but we tried to start it again.
            # Notice how I didn't redefine the threading object here again, this is because this thread should NEVER
            # run more than once.
            try: self.safelyTerminateProcessWorkerThdObj.start()
            except RuntimeError:
                print(f"[WARNING] {FUNCTION_NAME}: Safe Process Termination Worker Thread is running but could not be detected by first detection stage, will ignore this event...")
            except Exception as __errStartingSPTWThd:
                tb = __errStartingSPTWThd.__traceback__
                bcodeFromTb: dis.Bytecode = dis.Bytecode.from_traceback(tb)
                print(f"[ERROR] {FUNCTION_NAME}: Failed to start Safe Process Termination Worker Thread: {__errStartingSPTWThd}; An error window with more details (and stack trace) will be shown to the user...")
                error.ErrorWindow(f"""An unhandled Exception has occured while attempting to start the Safe Process Termination Worker (SPTW) Thread:
Error details:
{__errStartingSPTWThd}
...
""")
        ...
        def updateBannerTmpFSizes() -> bool:
            """
            ...
            **PLEASE IF YOU PLAN TO USE THIS FUNCTION, RUN IT UNDER A DIFFERENT THREAD ASIDE FROM THE MAIN ONE**
            ...
            if updater.readVersion() == None:
                ...
                if currentUserName == __SYSTEM_USER:
                    ...
                    # we must acquire the sharedObjsCSectionsLocker and release it in the same statement.
                    while self.sharedObjsCSectionsAccessLocker.acquire() and (self.sharedObjsCSectionsAccessLocker.release() or True) and self.__runSysUsrWarnAnimation:
                        ...
                        with self.sharedObjsCSectionsAccessLocker:
                            if self.__runSysUsrWarnAnimation:
...
""")
        ...
        self.about_window_btn = animated_widgets.HoverShrinkAnimatedButton(self.mainButtonsContainerFrame, buttonText=getCurrentLanguage().about_text, buttonCommand=startAboutWindow, width=369, height=108)
        self.about_window_btn.grid(column=0, row=1, padx=10)
        ...
        self.exec_btn = animated_widgets.HoverShrinkAnimatedButton(self.mainButtonsContainerFrame, buttonText=getCurrentLanguage().execute_text, buttonCommand=multiprocessing_execute_btn_function, width=369, height=108)
        self.exec_btn.grid(column=1, row=1, padx=10)

        # daemon=True prevents joining the thread + ensures python terminates it 
        # whenever we raise a SystemExit
        ...
        # width = 369, height= 108
        self.config_page_btn = animated_widgets.HoverShrinkAnimatedButton(self.mainButtonsContainerFrame, buttonText=getCurrentLanguage().settings_text, buttonCommand=self.StartConfigurationWindow, width=369, height=108)
        self.config_page_btn.grid(column=2, row=1)
        ...
        try:
            ...
            if int(self.winfo_screenwidth()) <= 1024 and int(self.winfo_screenheight()) <= 768:
                ...
                self.about_window_btn.grid_configure(padx=3)
                self.exec_btn.grid_configure(padx=3)
                self.exec_btn.configure(width=self.__ACTIONBTNS_FRAME_LOWRES_WIDTH, height=self.__ACTIONBTNS_FRAME_LOWRES_HEIGHT)
                self.config_page_btn.configure(width=self.__ACTIONBTNS_FRAME_LOWRES_WIDTH, height=self.__ACTIONBTNS_FRAME_LOWRES_HEIGHT)
                self.about_window_btn.configure(width=self.__ACTIONBTNS_FRAME_LOWRES_WIDTH, height=self.__ACTIONBTNS_FRAME_LOWRES_HEIGHT)
            # ----------------------------
...
""")
        ...
        try:
            ControllableThread(target=checkForUpdatesAutomatically).start()
        ...
        self.updBannerTmpFsThd: ControllableThread = ControllableThread(target=updateBannerTmpFSizes, name="FSCHECK_BANNER_UPDATE_THREAD0", daemon=False)
        self.updBannerTmpFsThd.start()
        # running a new thread for checking if the user is running under SYSTEM or not and to update infobanner widget.
        self.chkIfUsingSysUsrThd: ControllableThread = ControllableThread(target=checkIfRunningAsSYSTEMUser, name="SYSUSRCHECK_BANNER_UPDATE_THREAD0", daemon=False)
        self.chkIfUsingSysUsrThd.start()
        ...
        # a function for handling the destroy event (safely).
        self.__safeDestroyEvtHandlerThdName: str = "SAFE_MAIN_WINDOW_DESTRUCTION_EVT_HANDLER_THD0"
        # a thread object for safe destruction of the main window UI.
        self.__safeDestroyEvtHandlerThdObj: ControllableThread = ControllableThread(target=self.__safe_destroy_evt_handler, name=self.__safeDestroyEvtHandlerThdName, daemon=False)
        # the Thread used for calling the function that will handle the tasks for safely terminating the program's process.
        self.safelyTerminateProcessWorkerThdName: str = "SAFELY_TERMINATE_PROG_PROCESS_WORKER"
        self.safelyTerminateProcessWorkerThdObj: ControllableThread = ControllableThread(target=__safely_terminate_program_process_worker, name=self.safelyTerminateProcessWorkerThdName, daemon=False)
        # can destroy self event
        # binding shutdown poller function.
        self.after(500, __poll_shutdown_evt)

    def __safe_destroy_evt_handler(self) -> None:
        FUNCTION_NAME: str = "__safe_destroy_evt_handler"
        """
        A method that will handle the destruction of animated widgets for the main window UI screen.

        **SINCE THIS METHOD WILL JOIN THREADS, IT MUST BE EXECUTED WITHIN A DIFFERENT THREAD (ASIDE FROM THE MAIN THREAD)**
        """
        # okay so here's what this function will be doing.
        # 1-Iterating over every HoverShrinkAnimatedButton widget in the main Window UI screen
        # 2-Calling its own overridden destroy() method.
        # 3-The destroy() overridden method runs a thread that ensures safe termination of all animation threads
        # therefore we have to join that thread for each HoverShrinkAnimatedButton widget we find (iterate over).
        # 4-After then, we can set the destroy event for this main window screen so that the poller function will
        # safely call destroy on the main window screen UI.
        print(f"[DEBUG] {threading.current_thread().name}:{threading.current_thread().ident}@{FUNCTION_NAME}: execution begin...")
        __objs_names_in_self: list[str] = dir(self)
        for __obj_name in __objs_names_in_self:
            # we have to see if the current attribute found is a HoverShrinkAnimatedButton
            if type(getattr(self, __obj_name, None)) is animated_widgets.HoverShrinkAnimatedButton:
                # this is a HoverShrinkAnimatedButton, we can continue.
                print(f"[DEBUG] {threading.current_thread().name}:{threading.current_thread().ident}@{FUNCTION_NAME}: object: self.{__obj_name} is a HoverShrinkAnimatedButton!")
                # let's call it's destroy method.
                try: getattr(getattr(self, __obj_name),"destroy")()
                except Exception as __errCallingWidgetDestroyMthd:
                    print(f"[ERROR] {threading.current_thread().name}:{threading.current_thread().ident}@{FUNCTION_NAME}: Failed to call destroy() for object self.{__obj_name}: {__errCallingWidgetDestroyMthd}")
                    continue
                # let's join that thread that the destroy() method starts.
                if getattr(getattr(getattr(self, __obj_name),"_HoverShrinkAnimatedButton__setSafeDestroyFlagThdObj"),"is_alive")(): # nested getattr, sorry.
                    print(f"[DEBUG] {threading.current_thread().name}:{threading.current_thread().ident}@{FUNCTION_NAME}: thread: self.{__obj_name}.__setSafeDestroyFlagThdObj is alive, joining it...")
                    # apparently, if your attribute name starts with '__', Python tends to save its name
                    # as _[class name][your attribute] so they don't collide with sub-classes.
                    getattr(getattr(getattr(self, __obj_name), "_HoverShrinkAnimatedButton__setSafeDestroyFlagThdObj"), "join")()
        
        print(f"[DEBUG] {threading.current_thread().name}:{threading.current_thread().ident}@{FUNCTION_NAME}: It is now safe to destroy the main window UI!")
        # after we join all the safe destruction threads for every animated widget, we can now set our safe destroy flag.
        self.__canDestroySelf.set()
        return None
    ...
    def destroy(self) -> None:
        ...
        """
        Base class CTk destroy() method override for thread safety.
        """
        # we need to destroy all animated widgets instances first.
        print(f"[DEBUG] {threading.current_thread().name}:{threading.current_thread().ident}@{FUNCTION_NAME}: execution started...")
        # let's start our safe destroy handler thread.
        __thds_running: list[threading.Thread] = threading.enumerate()
        for __thd in __thds_running:
            print(f"[DEBUG] {threading.current_thread().name}:{threading.current_thread().ident}@{FUNCTION_NAME}: thread: {__thd.name}")
            if str(__thd.name) == self.__safeDestroyEvtHandlerThdName:
                return None
        # thread was not found to be running.
        # let's redefine its object (just in case).
        self.__safeDestroyEvtHandlerThdObj = ControllableThread(target=self.__safe_destroy_evt_handler, name=self.__safeDestroyEvtHandlerThdName, daemon=False)
        try: self.__safeDestroyEvtHandlerThdObj.start()
        except RuntimeError:
            print(f"[DEBUG] {threading.current_thread().name}:{threading.current_thread().ident}@{FUNCTION_NAME}: safe destroy event handler thread was already running but could not be detected by first detection stage, can't continue!")
            return None
        except Exception as __errStartingSafeDestroyEvtHandlerThd:
            print(f"[ERROR] {threading.current_thread().name}:{threading.current_thread().ident}@{FUNCTION_NAME}: couldn't start safe destroy event handler thread: {__errStartingSafeDestroyEvtHandlerThd}")
            return None
...
class SettingsWindow(Toplevel):
    def __init__(self):
        ...
        self.closewindow_btn = animated_widgets.HoverShrinkAnimatedButton(self, buttonText=getCurrentLanguage().quit_settings_btn, buttonCommand=SelfDestroy)
        self.closewindow_btn.place(x=230, y=495, relwidth=0.20, relheight=0.060)

        self.applychangesandclose_btn = animated_widgets.HoverShrinkAnimatedButton(self, buttonText=getCurrentLanguage().commit_changes_plus_exit_btn, buttonCommand=SaveConfigurationandQuit)
...
class AboutWindow(Toplevel):
    def __init__(self):
        super().__init__()
        # an event to instruct us whether or not it is allowed to destroy self.
        self.__canDestroySelf: threading.Event = threading.Event()
        # clear that event obj
        self.__canDestroySelf.clear()
        # add a shutdown event poller function
        def __poll_shutdown_evt():
            if self.__canDestroySelf.is_set():
                return __class__.__bases__[0].destroy(self) # call unmodified destroy function.
            self.after(500, __poll_shutdown_evt)
            return None
        ...
        # end --------------------
        # width and height for all buttons
        BTNS_WIDTH: int = 140
        BTNS_HEIGHT: int = 30
        # declaring the close window button
        self.close_window_btn = animated_widgets.HoverShrinkAnimatedButton(self, buttonText=getCurrentLanguage().ok, buttonCommand=closeAboutWindow, width=BTNS_WIDTH, height=BTNS_HEIGHT)
        self.close_window_btn.place(x=10, y=200)

        self.showcpyrights_windowbtn = animated_widgets.HoverShrinkAnimatedButton(self, buttonText=getCurrentLanguage().open_license_window_btn_text, buttonCommand=showLicenseWindow, width=BTNS_WIDTH, height=BTNS_HEIGHT)
        self.showcpyrights_windowbtn.place(x=10, y=240)

        self.goto_website_btn = animated_widgets.HoverShrinkAnimatedButton(self, buttonText=getCurrentLanguage().goto_tcg_website, buttonCommand=opentcgwebsiteonbrowser, width=BTNS_WIDTH, height=BTNS_HEIGHT)
        self.goto_website_btn.place(x=10, y=280)

        self.runupdater_btn = animated_widgets.HoverShrinkAnimatedButton(self, buttonText=getCurrentLanguage().check_for_updates, buttonCommand=updateCheckFunction, width=BTNS_WIDTH, height=BTNS_HEIGHT)
        self.runupdater_btn.place(x=10, y=320)

        self.donatorswindow_btn = animated_widgets.HoverShrinkAnimatedButton(self, buttonText=getCurrentLanguage().donators_btn, buttonCommand=showDonatorsWindow, width=BTNS_WIDTH, height=BTNS_HEIGHT)
        self.donatorswindow_btn.place(x=10, y=360)

        self.launchstartuptips_btn = animated_widgets.HoverShrinkAnimatedButton(self, buttonText=getCurrentLanguage().tip_of_the_day, buttonCommand=launchTipOfTheDay, width=BTNS_WIDTH, height=BTNS_HEIGHT)
        self.launchstartuptips_btn.place(x=10, y=400)
        # ima place it here just in case it failed to load any of the pictures in the window.
        self.after(500, __poll_shutdown_evt)

        # declare a ControllableThread object here that will handle destruction of animations.
        self.__safeDestroyEvtHandlerThdName: str = "SAFE_ABOUT_WINDOW_DESTROY_EVT_HANDLER_THREAD0"
        self.__safeDestroyEvtHandlerThdObj: ControllableThread = ControllableThread(target=self.__safe_destroy_evt_handler, name=self.__safeDestroyEvtHandlerThdName, daemon=False)
    ...
    def __safe_destroy_evt_handler(self) -> None:
        FUNCTION_NAME: str = "__safe_destroy_evt_handler"
        # list of strings representing names of all attributes in self.
        __objs_names_in_self: list[str] = dir(self)
        for __obj_name in __objs_names_in_self:
            if type(getattr(self, __obj_name, None)) is animated_widgets.HoverShrinkAnimatedButton:
                # the attribute name under __obj_name is a HoverShrinkAnimatedButton widget.
                # we must call its destroy method.
                try: getattr(getattr(self, __obj_name),"destroy")()
                except Exception as __errCallingWidgetDestroy:
                    print(f"[ERROR] {threading.current_thread().name}:{threading.current_thread().ident}@{FUNCTION_NAME}: Failed to call destroy() method of object self.{__obj_name}: {__errCallingWidgetDestroy}")
                    return None

                # we must check if the thread object spawned by the destroy() method of the animated widget is alive or not.
                if getattr(getattr(getattr(self, __obj_name), "_HoverShrinkAnimatedButton__setSafeDestroyFlagThdObj"),"is_alive")():
                    print(f"[DEBUG] {threading.current_thread().name}:{threading.current_thread().ident}@{FUNCTION_NAME}: safe destroy method of self.{__obj_name}.__setSafeDestroyFlagThdObj is alive, joining it...")
                    # we must join that thread...
                    getattr(getattr(getattr(self, __obj_name), "_HoverShrinkAnimatedButton__setSafeDestroyFlagThdObj"),"join")()
        
        print(f"[DEBUG] {threading.current_thread().name}:{threading.current_thread().ident}@{FUNCTION_NAME}: It is now safe to destroy the About window!")
        # set flag.
        self.__canDestroySelf.set()
        return None
    ...
    def destroy(self) -> None:
        FUNCTION_NAME: str = "destroy"
        """
        destroy() method override for safe destruction of threads of animated widgets.

        WARNING: code and statements inside this method override will be executed inside the MainThread or the
        Thread responsible for displaying the UI of this window, so better be careful!
        """
        print(f"[DEBUG] destroy()@{self.__str__}: execution started...")
        # list all currently active threads using threading.enumerate()
        __thds_objs: list[threading.Thread] = threading.enumerate()
        # we have to check if our targetted thread is already running or not.
        for __thd_obj in __thds_objs:
            print(f"[DEBUG] destroy()@{self.__str__}: Thread {str(__thd_obj.name)}")
            if str(__thd_obj.name) == self.__safeDestroyEvtHandlerThdName:
                return None
        # at this point, we are 95% certain that our targetted thread is not running.
        # we will redefine the ControllableThread object just in case.
        self.__safeDestroyEvtHandlerThdObj = ControllableThread(target=self.__safe_destroy_evt_handler, name=self.__safeDestroyEvtHandlerThdName, daemon=False)
        # Start that thread's activity (but with Exception handling!)
        try:
            self.__safeDestroyEvtHandlerThdObj.start()
        except RuntimeError:
            print(f"[WARNING] destroy()@{self.__str__}: safe destruction event handler thread is already running but not detected by first detection stage, can't continue!")
            return None
        except Exception as __errStartingThd:
            print(f"[ERROR] destroy()@{self.__str__}: Failed to start safe destruction event handler thread: {__errStartingThd}")
            return None
        return None
