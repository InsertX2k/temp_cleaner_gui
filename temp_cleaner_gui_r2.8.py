...
root.title("The Temp_Cleaner GUI Project (v2.8) - OutputConsole Update")
...
def show_about_window():
    messagebox.showinfo("About The Temp_Cleaner GUI Project (v2.8) - OutputConsole Update","""The Temp_Cleaner GUI Project by Insertx2k Dev\n
Version 2.8
Update 10
Written by Insertx2k Dev (Mr.X)
Support Twitter : @insertplayztw
\n
In this update of The Temp_Cleaner GUI Project, You will no longer see lots of command prompt windows into your screen, we've moved all of them into the new Output Console Window.
...
def show_output():
    ...
    def disable_event():
        try:
            root.destroy()
        except Exception as excpt2912:
            pass

        try:
            output_root.destroy()
        except Exception as excpt34257:
            pass

        return True

    clr_console(1)
    output_root.bind("<F6>", clr_console)
    

    # Defining the root protocol.
    output_root.protocol("WM_DELETE_WINDOW", disable_event)
