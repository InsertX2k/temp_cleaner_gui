    ...
    def __init__(self):
            ...
            try:
                ...
                if str(GetConfig['ProgConfig']['languagesetting']) == "en":
                    if convert_to_str_with_units(freed_up) == "0 bytes":
                        messagebox.showinfo(en.freed_up_diskspace_dialog_title, en.freed_up_nothing)
                    else:
                        messagebox.showinfo(en.freed_up_diskspace_dialog_title, f"{en.freed_up_diskspace_dialog}{convert_to_str_with_units(freed_up)}")
                elif str(GetConfig['ProgConfig']['languagesetting']) == "ar":
                    if convert_to_str_with_units(freed_up) == "0 bytes":
                        messagebox.showinfo(ar.freed_up_diskspace_dialog_title, ar.freed_up_nothing)
                    else:
                        messagebox.showinfo(ar.freed_up_diskspace_dialog_title, f"{convert_to_str_with_units(freed_up)}{ar.freed_up_diskspace_dialog}")
                else:
                    if convert_to_str_with_units(freed_up) == "0 bytes":
                        messagebox.showinfo(en.freed_up_diskspace_dialog_title, en.freed_up_nothing)
                    else:
        ...
        def show_used_systemdrive_space(keybinding_arg):
            ...
            try:
                ...
                if str(GetConfig['ProgConfig']['languagesetting']) == "en":
                    messagebox.showinfo(en.usage_statistics_for_systemdrive_word, f"""{en.usage_statistics_for_sysdrv}{system_drive.replace(":", '')}:
{en.total_systemdrive_space}{convert_to_str_with_units(total)}
{en.free_systemdrive_space}{convert_to_str_with_units(free)}
{en.used_systemdrive_space}{convert_to_str_with_units(used)}

{en.press_ok_when_youre_done}
""")
                elif str(GetConfig['ProgConfig']['languagesetting']) == "ar":
                    messagebox.showinfo(ar.usage_statistics_for_systemdrive_word, f""":{system_drive.replace(":", '')}{ar.usage_statistics_for_sysdrv}
{convert_to_str_with_units(total)}{ar.total_systemdrive_space}
{convert_to_str_with_units(free)}{ar.free_systemdrive_space}
{convert_to_str_with_units(used)}{ar.used_systemdrive_space}

{ar.press_ok_when_youre_done}
""")
                else:
                    messagebox.showinfo(en.usage_statistics_for_systemdrive_word, f"""{en.usage_statistics_for_sysdrv}{system_drive.replace(":", '')}:
{en.total_systemdrive_space}{convert_to_str_with_units(total)}
...
""")
        ...
        try:
            ...
            if str(GetConfig['ProgConfig']['languagesetting']) == "en":
                messagebox.showerror(en.exception_reading_thememode_title, f"{en.exception_reading_thememode_content}\n{exception_reading_theme}")
            elif str(GetConfig['ProgConfig']['languagesetting']) == "ar":
                messagebox.showerror(ar.exception_reading_thememode_title, f"{ar.exception_reading_thememode_content}\n{exception_reading_theme}")
            else:
                messagebox.showerror(en.exception_reading_thememode_title, f"{en.exception_reading_thememode_content}\n{exception_reading_theme}")
