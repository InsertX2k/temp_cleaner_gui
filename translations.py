...
# THIS IS DONE AUTOMATICALLY BY THE BUILD SCRIPT, DO NOT MODIFY IT
# module build_date is automatically generated by the build script!
try: BUILD_DATE = importlib.import_module("build_date").BUILD_DATE
except: pass
class en:
    build_date_str = "Built at : "
    try: build_date_full_str: str = build_date_str + BUILD_DATE
    except: build_date_full_str: str = ''
    __str__ = "en"
    """
...
"""
    ...
    roblox_no_versions_folders = "Your current Roblox installation is corrupt, there aren't any versions installed!"
    failed_to_acquire_wakelock = "Failed to acquire a System Request Power Control, This PC can go into sleep mode or any other low-power state during the cleaning session."
    acquired_wakelock = "Successfully Acquired a Wakelock!"
    released_wakelock = "Successfully Released a Wakelock!"
    wakelock_released_but_handle_not_closed = "Successfully Released the Wakelock but the Power Control Handle could not be closed..."
    failed_to_release_wakelock = "Failed to Release Wakelock!!!"
    wakelock_settings = "Wakelock Settings"
    acquire_wakelock_during_cleanup = "Acquire a Wakelock to prevent system from going into low-power state during cleaning"
    sys_usr_warn_header = "Running as a High Privileged User"
    sys_usr_warn_content_txt = "Running this program under a user with the same privileges as TrustedInstaller or higher can cause SERIOUS damages to your OS if not used with caution, YOU HAVE BEEN WARNED!"
    additional_cleaners = "Additional Cleaning Options"



class ar:
    build_date_str = " : تم البناء في  "
    try: build_date_full_str: str = build_date_str + BUILD_DATE
    except: build_date_full_str: str = ''
    __str__ = "ar"
    """
...
"""
    ...
    roblox_no_versions_folders = "النسخة الحالية من روبلوكس غير مثبتة بشكل صحيح، لا يوجد اي مجلد لملفات اللعبة"
    failed_to_acquire_wakelock = "فشل في إرسال طلب التحكم في وضع الطاقة، هذا الكمبيوتر يستطيع ان يدخل في وضع النوم او اي  وضع توفير طاقة اخر حتي اثناء عملية التنظيف"
    acquired_wakelock = "تم طلب قفل الإبقاء مستيقظا"
    released_wakelock = "تم فك قفل الإبقاء مستيقظا بنجاح"
    wakelock_released_but_handle_not_closed = "...تم بنجاح إلغاء قفل الإبقاء مستيقظا ولاكن لم يتم إغلاق عنصر مدير الطاقة"
    failed_to_release_wakelock = "!!!فشل في إلغاء قفل الإبقاء مستيقظا"
    wakelock_settings = "ضبط إلغاء قفل الإبقاء مستيقظا"
    acquire_wakelock_during_cleanup = "ﻒﻴﻈﻨﺘﻟﺍ ﺔﻴﻠﻤﻋ ﺀﺎﻨﺛﺃ ﺔﻗﺎﻃ ﺮﻴﻓﻮﺗ ﻊﺿﻭ ﻱﺍ ﻲﻓ ﻝﻮﺧﺪﻟﺍ ﻦﻣ ﻡﺎﻈﻨﻟﺍ ﻊﻨﻤﻟ ﺎﻈﻘﻴﺘﺴﻣ ﺀﺎﻘﺑﻹﺍ ﻞﻔﻗ ﺮﻣﺃ ﺐﻠﻃ"
    sys_usr_warn_header = "يعمل كمستخدم عالي الصلاحيات"
    sys_usr_warn_content_txt = """هذا البرنامج يعمل بصلاحيات مستخدم اعلي من او تساوي صلاحيات TrustedInstaller
إستمرارك في تشغيل البرنامج بهذه الصلاحيات قد يتسبب في ضرر ملحوظ لنظامك إن لم تكن حريص في إستخدامك له
"""
    additional_cleaners = "خيارات التنظيف الإضافية"