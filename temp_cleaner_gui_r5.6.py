...
""")
        ...
        if int(self.winfo_screenwidth()) <= 1024 and int(self.winfo_screenheight()) <= 768:
            ...
            self.banner = PhotoImage(file="bannerlowres.png")
        ...
        def incrFontSizeCoutput(keybinding_arg):
            ...
            print(f"current widget font size is : {font_size}")
        ...
        def decrFontSizeCoutput(keybinding_arg):
            ...
            """
            global font_size, GetConfig
            print(f"current widgets font size is: {font_size}")
            if int(font_size) == 4:
...
""")
        ...
        try:
            if int(GetConfig["ProgConfig"]['autocheckforupdates']) == 1: # auto check for updates is enabled
                try:
                    updater.downloadAndSaveVersionFile() # downloading version file from server
                    updater.downloadAndSaveReleaseNotesFile() # downloading release notes file from server
                    isThereAnyUpdatesAvailable = updater.readVersion()
                ...
                except Exception as exception_checkingforupdates:
                    messagebox.showerror("An error has occured", f"Couldn't either read from Updates server or display the updater program UI\nError details are:\n{exception_checkingforupdates}\n\nIf you are seeing this message as a user we strongly recommend that you create an issue on Temp_Cleaner GUI's Official Github repository at https://github.com/insertx2k/temp_cleaner_gui with a screenshot of this message\nPress OK to continue.")
            else: # auto check for updates is disabled
                pass
        except Exception as exception_reading_autoupdate_option:
            messagebox.showerror("An error has occured", f"Unable to read the option to check for automatic updates upon startup, the program will not check for updates and continue functioning as expected\nError details are:\n{exception_reading_autoupdate_option}")
        ...
        def RetrieveConfig():
            ...
            try:
                ...
                if str(self.RetrieveConfig_Init['ProgConfig']['autocheckforupdates']) == "1": # enabled
                    ...
                    print(f"autocheckforupdates: {self.checkforupdatesautomatically.get()}")
                else: # disabled
...
class AboutWindow(Toplevel):
    def __init__(self):
        ...
        def updateCheckFunction():
            ...
            """
            print("User has manually initializied the UpdateCheck function\nThe program will display the updater window.")
            updater.downloadAndSaveVersionFile() # download version file from server
            updater.downloadAndSaveReleaseNotesFile() # download release notes file from server.
